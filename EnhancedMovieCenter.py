...
def setEPGLanguage(dummy=None):
	if config.EMC.epglang.value:
		spDebugOut("Setting EPG language: " + str(config.EMC.epglang.value))
...
def setupKeyResponseValues(dummy=None):
	...
	s1 = struct.pack("LLHHl", 0, 0, 0x14, 0x00, config.EMC.key_repeat.value)
	s2 = struct.pack("LLHHl", 0, 0, 0x14, 0x01, config.EMC.key_period.value)
...
def EMCStartup(session):
	if not os.path.exists(config.EMC.folder.value):
		spTasker.shellExecute("mkdir " + config.EMC.folder.value)
	spDebugOut("+++ EMC "+EMCVersion+" startup")
	...
	if os.path.exists(config.EMC.folder.value + "/EMC_standby_flag.tmp"):
		...
		spTasker.shellExecute("rm -f " + config.EMC.folder.value + "/EMC_standby_flag.tmp")
...
#		self["key_blue"] = Button(" ")
		...
		self.list.append(getConfigListEntry(_("Movie trashcan path"), config.EMC.movie_trashpath, self.validatePath, self.openLocationBox))
		...
		self.list.append(getConfigListEntry(_("Daily trashcan cleanup time"), config.EMC.movie_trashcan_ctime, trashCleanSetup))
		self.list.append(getConfigListEntry(_("Delete validation ?"), config.EMC.movie_trashcan_validation, None, None))
		self.list.append(getConfigListEntry(_("EMC desc field update delay"), config.EMC.movie_descdelay, None, None))
		self.list.append(getConfigListEntry(_("Sort file A to Z at startup"), config.EMC.CoolStartAZ, self.needsRestart, None))
		self.list.append(getConfigListEntry(_("EMC file order reverse"), config.EMC.moviecenter_reversed, None, None))
		...
		self.list.append(getConfigListEntry(_("EMC cursor predictive move after selection"), config.EMC.moviecenter_selmove, None, None))
		self.list.append(getConfigListEntry(_("EMC show movie icons"), config.EMC.movie_icons, None, None))
		self.list.append(getConfigListEntry(_("EMC show icon indication for non-watched"), config.EMC.movie_mark, None, None))
		self.list.append(getConfigListEntry(_("EMC try to load titles from .meta files"), config.EMC.movie_metaload, None, None))
		...
		self.list.append(getConfigListEntry(_("EMC display directory reading text"), config.EMC.moviecenter_loadtext, None, None))
		self.list.append(getConfigListEntry(_("EMC always reload after open"), config.EMC.movie_reload, None, None))
		self.list.append(getConfigListEntry(_("EMC show Cut-Nr if exist"), config.EMC.CoolMovieNr, self.needsRestart, None))
		self.list.append(getConfigListEntry(_("EMC blue button function"), config.EMC.movie_bluefunc, None, None))
		self.list.append(getConfigListEntry(_("EMC hide movies being moved"), config.EMC.movie_hide_mov, None, None))
		self.list.append(getConfigListEntry(_("EMC hide movies being deleted"), config.EMC.movie_hide_del, None, None))
		self.list.append(getConfigListEntry(_("EMC hide trashcan directory"), config.EMC.movie_trashcan_hide, None, None))
		self.list.append(getConfigListEntry(_("EMC hide VLC directory"), config.EMC.movie_vlc_hide, None, None))
		...
		self.list.append(getConfigListEntry(_("Key period value (30-900)"), config.EMC.key_period, setupKeyResponseValues, None))
	...
	def onDialogShow(self):
		self.setTitle("Enhanced Movie Center "+ EMCVersion)
	...
	def dbgChange(self, value):
		if value == True:
			...
			spTasker.shellExecute("rm -f " + config.EMC.folder.value + config.EMC.debugfile.value)
