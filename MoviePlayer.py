...
def MovieCenterState(state):
	global gPlayerOpenedList
	gPlayerOpenedList = state
	#spDebugOut("[spPlayer] MovieCenterState = %s" %str(state))
	...
	def __del__(self):
		try:
			...
			if gStopped:
				...
				if gPlayerOpenedList or config.EMC.movie_reopenEOF.value: # did the player close while movie list was open?
					DelayedFunction(80, gMS.session.execDialog, gMS)		# doesn't crash Enigma2 subtitle functionality
			MovieCenterState(False)
		except Exception, e:
	...
	def evEOF(self, needToClose=False):
		...
		if (self.playcount+1) < len(self.playlist):
			...
			if 0:
				...
				if needToClose or config.usage.on_movie_eof.value != "pause":	# ask, MovieCenter, quit or pause
