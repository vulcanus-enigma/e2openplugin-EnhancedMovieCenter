...
playerMP3 = audioExt | videoExt - playerDVB - playerDVD		# ServiceMP3 GStreamer
...
class MovieCenter(GUIComponent, VlcPluginInterfaceList):
	...
	def getProgress(self, service, len=0, last=0, forceRecalc=False):
		...
		try:
			...
			if last > 0 or forceRecalc:
				...
				if len:
					...
					# Calculate progress
					progress = int(float(last) / float(adjlength) * 100.0)
					# Normalize progress
	...
	def invalidateService(self, service):
		self.l.invalidateEntry( self.getIndexOfService(service) ) # force redraw of the item
	...
	def getPlayerService(self, path, name, ext):
