...
#				"bYellowL":	(self.unUsed,				"-"),
				...
				"bArrowR":	(self.CoolKeyForward,		_("Directory forward")),
#				"bArrowRL":	(self.unUsed,				"-"),
				"bArrowL":	(self.CoolKeyBack,			_("Directory back")),
...
#				"bTEXTL":	(self.unUsed,				"-")
	...
	def CoolKeyForward(self):
		if CoolMemory != None:
			self.changeDir(CoolMemory)

	def CoolKeyBack(self):
		self.CoolBack( self["list"].getCurrentSelDir() )

	def CoolBack(self, nextdir):
		moveToIdx = 0
		global CoolMemory
		CoolMemory = self.currentPathSel
	...
	def keyPress(self):		# these only catch the key-Up events, not key-Down or repeat
		self.cursorDir = 0
		DelayedFunction(config.EMC.movie_descdelay.value, self.updateAfterKeyPress)
	def keyPressU(self):
		self.cursorDir = -1
		DelayedFunction(config.EMC.movie_descdelay.value, self.updateAfterKeyPress)
	def keyPressD(self):
	...
	def onDialogShow(self):
		if self.wasClosed:
			...
			if config.EMC.movie_reload.value or self["list"].newRecordings or self["list"].__len__() == 0:
	...
	def blueFunc(self):
		if config.EMC.movie_bluefunc.value == "Movie home": self.changeDir(config.EMC.movie_homepath.value)
		elif config.EMC.movie_bluefunc.value == "Play last": self.playLast()
	...
	def openPlayer(self, playlist):
		if self.playerInstance is None or self.moviesToDelete is None:
			Notifications.AddNotification(MoviePlayerSP, playlist, self)
		else:
			DelayedFunction(100, self.playerInstance.movieSelected, playlist)
	...
	def delPathSel(self, path):
		if path != "..":
			...
			if os.path.islink(path):
				spTasker.shellExecute("rm -f '" + path + "'")
				return True
	...
	def scriptCB(self, result=None):
		...
		env = "export SP_OUTDIR=%s"%config.EMC.folder.value
		env += " SP_HOME=%s"%config.EMC.movie_homepath.value
		env += " SP_PATH_LIMIT=%s"%config.EMC.movie_pathlimit.value
		env += " SP_TRASH=%s"%config.EMC.movie_trashpath.value
		env += " SP_TRASH_DAYS=%s"%config.EMC.movie_trashcan_limit.value
	...
	def rogueFiles(self):
		...
		spTasker.shellExecute( check.getScript(config.EMC.movie_trashpath.value) )
		self.session.open(MessageBox, check.getStatistics(), MessageBox.TYPE_INFO)

	def deleteFile(self, permanently=False):
		if self.browsingVLC(): return
		self.permanentDel  = permanently or config.EMC.movie_trashcan_limit.value == 0
		self.permanentDel |= self.currentPathSel == config.EMC.movie_trashpath.value
		self.permanentDel |= mountpoint(self.currentPathSel) != mountpoint(config.EMC.movie_trashpath.value)
...
#					self.delCurrentlyPlaying = nowPlaying in selectedlist
			...
			if entrycount == 1:
				...
				if not self.delCurrentlyPlaying:
					if config.EMC.movie_trashcan_validation.value or config.EMC.movie_trashcan_limit.value == 0 or self.permanentDel:
	...
	def deleteMovieConfimation(self, confirmed):
		if confirmed and self.moviesToDelete is not None and len(self.moviesToDelete)>0:
			...
			delete = config.EMC.movie_trashcan_limit.value==0 or self.permanentDel
			if os.path.exists(config.EMC.movie_trashpath.value) or delete:
				...
				self.execFileOp(config.EMC.movie_trashpath.value, self.moviesToDelete, op="delete", purgeTrash=delete)
		...
		for service in selectedlist:
			...
			if name is not None:
				if op=="delete":	# target == trashcan
					...
					if mountpoint(targetPath) != mountpoint(config.EMC.movie_homepath.value):		# CIFS to HDD is ok!
		...
		try:
			if os.path.exists(config.EMC.movie_trashpath.value):
				...
				for movie in dirlist:
					...
					currTime = localtime()
					expTime = localtime(os.stat(fullpath).st_mtime + 24*60*60*config.EMC.movie_trashcan_limit.value)
