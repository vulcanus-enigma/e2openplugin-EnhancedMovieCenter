...
class SelectionEventInfo:
	...
	def updateEventInfo(self):
		try:
			if self["list"].currentSelIsDirectory() or self["list"].currentSelIsVlc() or self.getCurrent() is None:
				self.resetEventInfo()
			else:
...
#				"EMCTEXTL":	(self.unUsed,				"-"),
	...
	def setNextPathSel(self, nextdir):
		...
		if nextdir == "..":
			...
			if os.path.exists(nextdir):
				...
				self.backStack.append((self.currentPathSel, self["list"].getCurrent()))
			self.changeDir(nextdir, moveToService)
	...
	def __toggleProgressService(self, service, preparePlayback, forceProgress=-1, first=False):
		try:
			...
			progress, len = self["list"].getProgress(service, len=len, last=last, forceRecalc=True)
			...
			if progress >= 100 or preparePlayback and progress >= int(config.EMC.movie_finished_percent.value):
				# 100% -> 0
				# Don't care about preparePlayback, always reset to 0%
				# else movie will start at the end
				cuts.toggleLastCutList(0) #, preparePlayback)
			elif progress <= 0 and not preparePlayback:
				# 0% -> SAVEDLAST or length
				cuts.toggleLastCutList(-1)
			elif not preparePlayback:
				# 1-99% -> length
				cuts.toggleLastCutList(1)
	...
	def scriptCB(self, result=None):
		...
		env += " SP_TRASH_DAYS=%s"%int(config.EMC.movie_trashcan_limit.value)
	...
	def purgeExpiredFromTrash(self):
		try:
			...
			emcDebugOut("[EMCMS] purgeExpiredFromTrash exception:\n" + str(e))
